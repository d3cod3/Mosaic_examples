...
void main() {
  ...
  vec4 origColor = texture(tex0, texCoordVarying);
  vec4 raysColor = texture(tex0, texCoordVarying);
  ...
  if (lightDirDOTviewDir>0.0){
    ...
    vec2 deltaTextCoord = vec2( texCoordVarying - lightPositionOnScreen);
    vec2 textCoo = texCoordVarying;
    ...
    for(int i=0; i < NUM_SAMPLES ; i++){
      ...
      vec4 tsample = texture(tex0, textCoo );
