...
void main(){
	...
	if (ns > 150.0){
		float shift = 30.0;
		col.r = texture2DRect(tex0, vec2(st.x-shift,st.y-shift)).r;
		col.b = texture2DRect(tex0, vec2(st.x-shift,st.y-shift)).b;
		col.g = texture2DRect(tex0, vec2(st.x-shift,st.y-shift)).g;
	}else{
